...
import com.example.SentimentScoreMapper;
import com.example.SentimentScoreReducer;
 ...
 */
public class SentimentScoreDriver extends Configured implements Tool {

    @Override
    public int run(String[] args) throws Exception {
        if (args.length < 2 || args.length > 3) {
            System.err.println("Usage: SentimentScoreDriver <input path> <output path> [lexicon path]");
            System.err.println("Example: SentimentScoreDriver /output/part-r-00000 /output-sentiment [hdfs:///lexicons/afinn.txt]");
            return -1;
        }
        ...
        job.setMapOutputKeyClass(SentimentScoreMapper.BookKey.class);
    ...
    public static void main(String[] args) throws Exception {
        int exitCode = ToolRunner.run(new SentimentScoreDriver(), args);
